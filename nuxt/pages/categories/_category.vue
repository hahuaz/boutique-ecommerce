      ...
      <div class="products">
        ...
        >
          <SfProductCard
            ...
            @click:wishlist="addToWishlist({ productId: product.id })"
            @click:add-to-cart="
...
export default {
  ...
  async fetch({ params: { category }, store }) {
    /* go to error page when error occurs */
  ...
  methods: {
    ...mapActions('cart', [
      'addToCart', // also supports payload `this.nameOfAction(amount)`
    ]),
    async addToWishlist(payload) {
      const result = await this.$store.dispatch('addToWishlist', payload)
      if (result === 'added') {
        this.$message({
